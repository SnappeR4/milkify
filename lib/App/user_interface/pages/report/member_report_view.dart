...
// Date range selection
                ...
                Center(
                  child: ElevatedButton(
                    onPressed: () {
                      if(controller.saleTransactions.isNotEmpty || controller.paymentTransactions.isNotEmpty){
  ...
  Future<Uint8List> _generateCombinedPdf() async {
    ...
    for (var i = 0; i < controller.saleTransactions.length; i += saleRecordsPerPage) {
    ...
    for (var i = 0; i < controller.paymentTransactions.length; i += paymentRecordsPerPage) {
    ...
    final int totalSaleMembers = controller.saleTransactions.map((t) => t.memberId).toSet().length;
    final double totalSaleLiters = controller.saleTransactions.fold(0.0, (sum, t) => sum + t.liters);
    final double totalSaleAmount = controller.saleTransactions.fold(0.0, (sum, t) => sum + t.total);

    // Calculate totals for payment
    final int totalBillCount = controller.paymentTransactions.length;
    final int totalPaymentMembers = controller.paymentTransactions.map((t) => t.memberId).toSet().length;
    final double totalPaidAmount = controller.paymentTransactions.fold(0.0, (sum, t) => sum + t.paidAmount);
          ...
          build: (context) {
            final children = <pw.Widget>[
              ...
              pw.Table.fromTextArray(
                headers: ['Date', 'Receipt No', 'Bill Type', 'Member ID', 'Opening Balance', 'Product ID', 'Rate', 'Liters', 'Total'],
              ...
              pw.SizedBox(height: 10),
              pw.Table.fromTextArray(
                headers: ['Total Receipt Count', 'Total Members', 'Total Liters', 'Grand Total Amount'],
                data: [
                  [totalSaleReceiptCount.toString(), totalSaleMembers.toString(), totalSaleLiters.toStringAsFixed(2), totalSaleAmount.toStringAsFixed(2)],
                ],
                headerStyle: pw.TextStyle(font: regularFont, fontSize: 10, fontWeight: pw.FontWeight.bold),
                cellStyle: pw.TextStyle(font: regularFont, fontSize: 9),
                cellAlignment: pw.Alignment.centerLeft,
        ...
        pw.Page(
          build: (context) {
            final children = <pw.Widget>[
              ...
              pw.Table.fromTextArray(
                headers: ['Total Bill Count', 'Total Members', 'Total Paid Amount'],
                data: [
                  [totalBillCount.toString(), totalPaymentMembers.toString(), totalPaidAmount.toStringAsFixed(2)],
                ],
                headerStyle: pw.TextStyle(font: regularFont, fontSize: 10, fontWeight: pw.FontWeight.bold),
                cellStyle: pw.TextStyle(font: regularFont, fontSize: 9),
                cellAlignment: pw.Alignment.centerLeft,
